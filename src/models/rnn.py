...
class RNNNet(RNNBaseModule):
    ...
    def forward(self, x: Tensor | PackedSequence, N: Tensor=None, batch_size: int=None) -> Tensor:
        ...
        if device != next(iter(self.rnn.parameters())).device:
            device = next(iter(self.rnn.parameters())).device
            x = x.to(device)
        
        dtype = x.data.dtype #if x.data.dtype == torch.float32 else torch.float32
        if dtype != next(iter(self.rnn.parameters())).dtype:
            dtype = next(iter(self.rnn.parameters())).dtype
            x = x.to(dtype)
