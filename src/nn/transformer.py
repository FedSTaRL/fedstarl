...
class TransformerBaseModule(BaseModule):
    def __init__(self, loss_kwargs: Dict[str, Any]={'loss_fn': 'sscl'}, *args, **kwargs):
    ...
    def loss(self, x: Tensor | PackedSequence | Dict[str, Any], y: Tensor, N: Tensor=None, batch_size: int=None, eval=False, forward: bool=False):
        self.eval() if eval else self.train()
        
        return_dict = {}
        if self._loss_fn == 'sscl': # not implemented
            if not forward: # x are raw inputs, hence need to be passed through forward pass 
                out_dict = self(x=x, N=N, mode='train')
                logits = out_dict['logits']
            else:
                assert isinstance(x, Dict)
                assert x.get('logits', None) is not None
                assert x.get('embedding_cls', None) is not None
                assert x.get('embedding_masked', None) is not None
                out_dict = x
                # x are logits
                logits = out_dict['logits']
            
            x_device = logits.device
            x_dtype = logits.dtype
            ...
            loss, loss_ce, loss_cl, loss_cl_batch_sim, loss_cl_class_sim = self.criterion[self._loss_fn](
                **loss_params)
            
            return_dict['loss'] = loss
            return_dict['loss_ce'] = loss_ce
            return_dict['loss_cl'] = loss_cl
            return_dict['loss_cl_batch_sim'] = loss_cl_batch_sim
            return_dict['loss_cl_class_sim'] = loss_cl_class_sim
            return return_dict
        
        elif self._loss_fn == LossOptions.bce:
            if not forward: # x are raw inputs, hence need to be passed through forward pass     
                out_dict = self(x=x, N=N, mode='train')
                logits = out_dict['logits']
            else:
            ...
            assert y.device == x_device, f'{y.device, x_device}'
            return {'loss_ce': self.criterion[self._loss_fn](logits.flatten(), y.type(x_dtype).flatten())}
        else:
            raise RuntimeError
    ...
    def metrics(self, x: Tensor | PackedSequence, y: Tensor, N: Tensor=None, batch_size: int=None, eval=False, cm_text_lables: List[str | int]=[0,1]):
        out_dict = self(x, N=N, batch_size=batch_size)
        num_samples = len(y)

        loss_dict = self.loss(out_dict, y, eval=eval, forward=True)
        y_preds = self._compute_predictions_from_logits(out_dict['logits'])
        if len(y_preds.shape) > 1:
            y_preds = y_preds.flatten()
            y = y.flatten()
        
        device = y.device
        if device != out_dict['logits'].device:
            device = out_dict['logits'].device
            y = y.to(device)
        ...
        if self._loss_fn == 'sscl': # not implemented
            return {
                'loss': Weighted(loss_dict['loss'].item(), 1),
                'loss_ce': Weighted(loss_dict['loss_ce'].item(), 1),
                'loss_cl': Weighted(loss_dict['loss_cl'].item(), 1),
                'loss_cl_batch_sim': Weighted(loss_dict['loss_cl_batch_sim'].item(), 1),
                'loss_cl_class_sim': Weighted(loss_dict['loss_cl_class_sim'].item(), 1),
...
class TransformerEncoderLayer(nn.TransformerEncoderLayer):
    """
    Custom Tranformer Encoder Block Accessing the attention weights
    """
...
def attn_batch_mean_adjusted_for_padding(x: Tensor, N: Union[List[int], Tensor]):
    """
    Function that calculates the mean per batch adjusted for padded sequences 
